...
function argsFor(options, downloadsDir) {
  ...
  if (options.mode === "audio") {
    args.push("-f", "bestaudio/best", "-x", "--audio-format", options.audioFormat || "mp3", "--audio-quality", options.audioQuality || "192K");
  } else {
    args.push("-f", options.formatId || "bestvideo+bestaudio/best", "--merge-output-format", "mp4");
  }
